define([
    ...
    function Context(canvas, options) {
         ...
         * <code>dispose</code> will be invoked on each object in this object literal that has
    ...
    PickId.prototype.dispose = function() {
     ...
     * The ID has an RGBA color value unique to this context.  You must call dispose()
    ...
    Context.prototype.dispose = function() {
        // Destroy all objects in the cache that have a dispose method.
        var cache = this.cache;
        for (var property in cache) {
            if (cache.hasOwnProperty(property)) {
                var propertyValue = cache[property];
                if (defined(propertyValue.dispose)) {
                    propertyValue.dispose();
                }
            }
        }

        this._shaderCache = this._shaderCache.dispose();
        this._defaultTexture = this._defaultTexture && this._defaultTexture.dispose();
        this._defaultCubeMap = this._defaultCubeMap && this._defaultCubeMap.dispose();
