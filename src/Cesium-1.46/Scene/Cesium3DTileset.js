define([
    ...
    function updateTiles(tileset, frameState) {
        ...
        if (tileset.debugShowGeometricError || tileset.debugShowRenderingStatistics || tileset.debugShowMemoryUsage || tileset.debugShowUrl) {
            ...
            tileset._tileDebugLabels = tileset._tileDebugLabels && tileset._tileDebugLabels.dispose();
        }
    }
     ...
     *
     * @example
     * tileset = tileset && tileset.dispose();
     *
     * @see Cesium3DTileset#isDestroyed
     */
    Cesium3DTileset.prototype.dispose = function() {
        this._tileDebugLabels = this._tileDebugLabels && this._tileDebugLabels.dispose();
        this._clippingPlanes = this._clippingPlanes && this._clippingPlanes.dispose();

        // Traverse the tree and dispose all tiles
        if (defined(this._root)) {
            var stack = scratchStack;
