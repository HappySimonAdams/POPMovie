define([
    ...
    PolylineCollection.prototype.remove = function(polyline) {
        if (this.contains(polyline)) {
            ...
            if (defined(polyline._bucket)) {
                ...
                bucket.shaderProgram = bucket.shaderProgram && bucket.shaderProgram.dispose();
                bucket.pickShaderProgram = bucket.pickShaderProgram && bucket.pickShaderProgram.dispose();
            }
            polyline._destroy();
            return true;
        }
    ...
    function createBatchTable(collection, context) {
        if (defined(collection._batchTable)) {
            collection._batchTable.dispose();
        }
     ...
     *
     * @example
     * polylines = polylines && polylines.dispose();
     *
     * @see PolylineCollection#isDestroyed
     */
    PolylineCollection.prototype.dispose = function() {
    ...
    function destroyVertexArrays(collection) {
        ...
        for ( var t = 0; t < length; ++t) {
            collection._vertexArrays[t].va.dispose();
        }
