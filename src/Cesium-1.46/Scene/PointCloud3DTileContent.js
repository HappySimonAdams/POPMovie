define([
    ...
    function createShaders(content, frameState, style) {
        ...
        if (defined(drawCommand.shaderProgram)) {
            ...
            drawCommand.shaderProgram.dispose();
        }
        ...
        if (defined(pickCommand.shaderProgram)) {
            // Destroy the old shader
            pickCommand.shaderProgram.dispose();
        }
    ...
    PointCloud3DTileContent.prototype.dispose = function() {
        var command = this._drawCommand;
        var pickCommand = this._pickCommand;
        if (defined(command)) {
            command.vertexArray = command.vertexArray && command.vertexArray.dispose();
            command.shaderProgram = command.shaderProgram && command.shaderProgram.dispose();
            pickCommand.shaderProgram = pickCommand.shaderProgram && pickCommand.shaderProgram.dispose();
        }
        this._batchTable = this._batchTable && this._batchTable.dispose();
