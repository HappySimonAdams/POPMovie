define([
    ...
    function ClippingPlaneCollection(options) {
        ...
        // If this ClippingPlaneCollection has an owner, only its owner should update or dispose it.
    ...
    ClippingPlaneCollection.setOwner = function(clippingPlaneCollection, owner, key) {
        // Don't dispose the ClippingPlaneCollection if it is already owned by newOwner
        if (clippingPlaneCollection === owner[key]) {
            return;
        }
        // Destroy the existing ClippingPlaneCollection, if any
        owner[key] = owner[key] && owner[key].dispose();
     ...
     *
     * @example
     * clippingPlanes = clippingPlanes && clippingPlanes .dispose();
     *
     * @see ClippingPlaneCollection#isDestroyed
     */
    ClippingPlaneCollection.prototype.dispose = function() {
        this._clippingPlanesTexture = this._clippingPlanesTexture && this._clippingPlanesTexture.dispose();
